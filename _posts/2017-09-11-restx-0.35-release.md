...
A new `@Param` annotation kind has been introduced : `Kind.HEADER` allowing to inject request headers into your Resource
endpoints.
...
- `@Header("foo")`, aliasing `@Param(value="foo", kind=Kind.HEADER)`
...
### Producing endpoint annotations metadata and make it available at runtime

Annotations attached to any `@RestxResource` endpoint will be transposed to `OperationDescription.annotations` field
of target endpoint.
...
Typically, it would allow to create any custom endpoint annotation, specific to your project, and implement your
custom behaviour corresponding to this custom annotation in a `RestxFilter`.
...
- you're tainting your endpoint with Restx specific API
- things you will do with `RestxResponse` instance will remain undocumented in api-docs
- with great powers come great responsibilities : if you use `RestxResponse` API to write `response` body too early,
  is may lead to unwanted/unpredictable behaviour.
...
- [6ce644f](https://github.com/restx/restx/commit/6ce644f) showing explicit error log when detecting DI cycle (see [682fb8a](https://github.com/restx/restx/commit/682fb8a))
...
- [424bedc](https://github.com/restx/restx/commit/424bedc) [34822ee](https://github.com/restx/restx/commit/34822ee) 
  [7389683](https://github.com/restx/restx/commit/7389683) [16a4951](https://github.com/restx/restx/commit/16a4951) better `ResourcesRoute` extensibility
...
- [6a56329](https://github.com/restx/restx/commit/6a56329) lowered factory log level from INFO to DEBUG for less verbose output
...
- [ba28876](https://github.com/restx/restx/commit/ba28876) `StdRestxSecurityManager` now takes a `PermissionFactory` in its constructor,
  if you subclassed it, you will need to provide this component during construction (simply inject it in your constructor).
...
- Some libraries have evolved :
  ...
  - [1c07f87](https://github.com/restx/restx/commit/1c07f87) following jong upgrade to 1.3.0, `mongo-java-driver` has been upgraded
...
- [46f8b58](https://github.com/restx/restx/commit/46f8b58) principal cached roles are now cleared upon logout; if you already
  were cleaning the roles, don't forget to remove this behaviour or implement your own `SessionInvalider` component for this purpose
...
- [3db7d18](https://github.com/restx/restx/commit/3db7d18) every graphite settings are now prefixed with `restx.` to avoid
  collisions with existing graphite properties potentially defined on app server 
...
- [489961d](https://github.com/restx/restx/commit/489961d) using static classes for deps command's subcommands instead
  of instance classes
- [449b870](https://github.com/restx/restx/commit/449b870) `DELETE` endpoint parameters should be looked up in `QUERY` 
  param instead of `BODY` content when no `@Param` is provided (default behaviour)
